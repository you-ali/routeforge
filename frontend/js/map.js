...
function buildIcon(type, color, label, isMid) {
  // Water-drop teardrop for intermediate waypoints
  if (isMid) {
  ...
  if(type==='flag'){
    // Checkered flag: white flag background with alternating dark squares (4×4 grid)
...
function getBearing(a,b){const r=Math.PI/180,y=Math.sin((b[1]-a[1])*r)*Math.cos(b[0]*r),x=Math.cos(a[0]*r)*Math.sin(b[0]*r)-Math.sin(a[0]*r)*Math.cos(b[0]*r)*Math.cos((b[1]-a[1])*r);return(Math.atan2(y,x)*180/Math.PI+360)%360}
...
function initMap(id) {
  ...
  // Mark tiles not-ready whenever the view moves (used by updatePosterPreview
  // to wait before capturing).  The middle map no longer auto-triggers a
  // preview update — the user refreshes the poster manually.
  map.on('movestart zoomstart',()=>{ window._tilesReady=false; });
  ...
  // Debounced poster-preview refresh — fires after the user stops dragging a
  // style slider so the preview stays in sync without hammering dom-to-image.
  function schedulePreview(){
    clearTimeout(window._previewTimer);
    window._previewTimer=setTimeout(()=>{
      if(window.currentRouteData&&window.updatePosterPreview) window.updatePosterPreview();
    },800);
  }

  // Map detail — affects only the visible map (CSS filter on tile pane)
  document.getElementById('map-detail-slider').addEventListener('input', e => {
    ...
    // No preview update — detail is a visible-map-only effect; capture map is unaffected
  });

  // Route width — redraws on both maps, then refreshes the preview
  ...
  // Map angle — CSS perspective on the visible map only; capture map unaffected
  ...
  angSlider.addEventListener('input', e => {
    ...
    // No preview update — angle is a visible-map-only effect
  });
...
function applyDrawMode() {
  ...
  if(on){btn.classList.add('active');document.getElementById('draw-label').textContent='Drawing Mode On';document.body.classList.add('draw-mode');banner.style.display='flex'}
  else{btn.classList.remove('active');document.getElementById('draw-label').textContent='Enable Draw Mode';document.body.classList.remove('draw-mode');banner.style.display='none'}
}
...
function switchTileUrl(url){
  ...
  // ── Capture map — same tiles, completely independent view ──
  if(captureMap){
...
// Hide only intermediate (midpoint) drag handles — start/end markers stay visible
// in the poster because they are part of the design.
...
// Renders route polyline + arrowheads into any layer group (used by both maps).
function renderRouteOnGroup(geojson, group){
  ...
  if(coords.length<=10){
    // Straight-line route — one arrow per segment at its midpoint
    for(let i=0;i<coords.length-1;i++) placeArrow(coords[i],coords[i+1]);
  } else {
    // Dense polyline (many interpolated points) — evenly spaced arrows
...
function drawRoute(geojson){
  ...
  // Mirror onto the capture map and fit it to the full route bounds.
  // The capture map is completely independent of the visible map's pan/zoom.
...
function placeEditMarkers(nodes,ss,es,onDrag){
  ...
  nodes.forEach((n,i)=>{
    ...
    // Mirror start/end icons onto the capture map (no drag handles on capture map)
...
window.clearRoute=clearRoute;window.fitRoute=fitRoute;
